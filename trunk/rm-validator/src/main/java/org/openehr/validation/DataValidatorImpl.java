...
public class DataValidatorImpl implements DataValidator {
	...
	void validateArchetypeInternalRef(Archetype archetype,
		...
		ArchetypeConstraint constraint = archetype.node(internalRef
				.getTargetPath());
	...
	private boolean validateRootSlot(ArchetypeSlot slot, Locatable locatable) {
		...
		if (slot.getExcludes() != null) {
			for (Assertion assertion : slot.getExcludes()) {
				ExpressionBinaryOperator operator = (ExpressionBinaryOperator) assertion
						.getExpression();
				ExpressionLeaf rightLeaf = (ExpressionLeaf) operator
						.getRightOperand();
	...
	private String getErrorDescription(Archetype archetype,
		...
		if (constraint instanceof CObject) {
			...
			nodeId = ((CSingleAttribute) constraint).getChildren().get(0)
					.getNodeId();
		}
		...
		if (nodeId != null && !nodeId.isEmpty()) {
			...
			List<OntologyDefinitions> ontologyDefinitions = ontology
